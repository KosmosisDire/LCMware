...
class CommandMsg(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(CommandMsg._get_packed_fingerprint())
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != CommandMsg._get_packed_fingerprint():
            raise ValueError("Decode error")
        return CommandMsg._decode_one(buf)
    ...
    def _get_hash_recursive(parents):
        if CommandMsg in parents: return 0
        newparents = parents + [CommandMsg]
    ...
    def _get_packed_fingerprint():
        if CommandMsg._packed_fingerprint is None:
            CommandMsg._packed_fingerprint = struct.pack(">Q", CommandMsg._get_hash_recursive([]))
        return CommandMsg._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", CommandMsg._get_packed_fingerprint())[0]
