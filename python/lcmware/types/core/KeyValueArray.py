...
class KeyValueArray(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(KeyValueArray._get_packed_fingerprint())
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != KeyValueArray._get_packed_fingerprint():
            raise ValueError("Decode error")
        return KeyValueArray._decode_one(buf)
    ...
    def _get_hash_recursive(parents):
        if KeyValueArray in parents: return 0
        newparents = parents + [KeyValueArray]
    ...
    def _get_packed_fingerprint():
        if KeyValueArray._packed_fingerprint is None:
            KeyValueArray._packed_fingerprint = struct.pack(">Q", KeyValueArray._get_hash_recursive([]))
        return KeyValueArray._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", KeyValueArray._get_packed_fingerprint())[0]
