...
class SystemStatusMsg(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(SystemStatusMsg._get_packed_fingerprint())
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != SystemStatusMsg._get_packed_fingerprint():
            raise ValueError("Decode error")
        return SystemStatusMsg._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = SystemStatusMsg()
    ...
    def _get_hash_recursive(parents):
        if SystemStatusMsg in parents: return 0
        newparents = parents + [SystemStatusMsg]
    ...
    def _get_packed_fingerprint():
        if SystemStatusMsg._packed_fingerprint is None:
            SystemStatusMsg._packed_fingerprint = struct.pack(">Q", SystemStatusMsg._get_hash_recursive([]))
        return SystemStatusMsg._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", SystemStatusMsg._get_packed_fingerprint())[0]
